...
const QueuePopCard = ({ p }: QueuePopCardProps) => {
  ...
  return (
    <div className='flex flex-col items-center justify-center content-center'>
      <img 
        className={cn(
          "w-12 h-12 border border-league-button", 
          p.accepted && "border border-green-500"
        )}
...
const QueuePop = () => {
  ...
  function handleAccept(){ socket?.emit("accept")}
  function handleDecline(){ socket?.emit("decline")}
      ...
      <div className={cn(
        ...
        "w-full h-full bg-black bg-opacity-90 z-[1000]"
          ...
          <div className='flex flex-col items-center justify-center content-center w-full max-w-xl '>
            <div className='font-beaufort-bold mt-6 mb-10 text-5xl'>
              Match Found
            </div>
            <div 
              className='block bg-text h-1 mb-2 transition-all ease-linear duration-[1000ms]'
              style={{ width: `${queuePop.timer / 3 * 10}%` }}
            />
