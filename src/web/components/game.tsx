...
const Game = ({ className, ...props }: GameProps) => {
  ...
  return (
    <div style={{ height: windowHeight }} className={cn(className, "flex flex-col flex-grow")} {...props}>
      ...
      <div className="w-full h-full flex gap-10 justify-center items-center content-center">

            <div className="flex flex-col items-end gap-2 w-full">
              {Object.keys(game.blue).map((key) => 
                <PlayerCard side="blue" className="max-w-md bg-neutral-800 border border-border shadow rounded-none" player={game.blue[key]} />
              )}
            </div>
            ...
            <div className="flex flex-col gap-2 w-full">
              {Object.keys(game.red).map((key) => 
                <PlayerCard side="red" className="max-w-md bg-neutral-800 border border-border shadow rounded-none" player={game.red[key]} />
              )}
            </div>
